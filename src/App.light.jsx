...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   White / grey / minimal  ·  Coolvetica typography
═══════════════════════════════════════════════════════════════════════════ */

// Coolvetica is a free condensed display font — load via CDN
const Styles = () => (
  <style>{`
    ...
    @font-face {
      ...
      src: url('https://cdn.jsdelivr.net/npm/@fontsource/barlow-condensed@5.0.8/files/barlow-condensed-latin-700-normal.woff2') format('woff2');
      font-weight: 700;
    }
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #c-ring { position:fixed;width:24px;height:24px;border-radius:50%;border:1px solid rgba(17,17,16,.25);pointer-events:none;z-index:99998;transform:translate(-50%,-50%); }
...
function Cursor() {
  ...
  useEffect(() => {
    ...
    const move = e => {
      ...
      const step = () => {
        rx = lerp(rx, e.clientX, 0.14); ry = lerp(ry, e.clientY, 0.14);
        if (ring.current) { ring.current.style.left = rx+'px'; ring.current.style.top = ry+'px'; }
        if (Math.abs(rx - e.clientX) > 0.5 || Math.abs(ry - e.clientY) > 0.5) raf = requestAnimationFrame(step);
      };
    ...
    <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
      <div style={{ width:'20px', height:'20px', background:'var(--black)', display:'flex', alignItems:'center', justifyContent:'center' }}>
        <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
          <path d="M2 4L5 6L2 8M10 4L7 6L10 8" stroke="white" strokeWidth="1.2" strokeLinecap="round"/>
        </svg>
      </div>
      <div>
        <div style={{ fontFamily:'var(--display)', fontSize:'13px', fontWeight:700, letterSpacing:'.18em', color:'var(--black)', lineHeight:1, textTransform:'uppercase' }}>American Hairline</div>
      </div>
    </div>
...
const Ticker = () => {
  ...
  return (
    <div style={{ overflow:'hidden', background:'var(--black)', padding:'9px 0', zIndex:10, position:'relative' }}>
      <div style={{ display:'flex', width:'max-content', animation:'ticker 30s linear infinite' }}>
        {[0,1,2,3].map(i => (
          <span key={i} style={{ fontFamily:'var(--body)', fontSize:'10px', fontWeight:400, letterSpacing:'.28em', textTransform:'uppercase', color:'rgba(255,255,255,.5)', whiteSpace:'nowrap', padding:'0 40px' }}>{t}</span>
        ))}
      </div>
    </div>
  );
  ...
  return (
    <section style={{
      ...
      <div style={{
        ...
        backgroundImage:'linear-gradient(var(--grey1) 1px,transparent 1px),linear-gradient(90deg,var(--grey1) 1px,transparent 1px)',
      ...
      <div style={{
        ...
        fontFamily:'var(--display)', fontSize:'clamp(220px,34vw,480px)', fontWeight:700,
        color:'rgba(17,17,16,.04)', lineHeight:1, pointerEvents:'none', userSelect:'none',
        letterSpacing:'-.03em', whiteSpace:'nowrap', textTransform:'uppercase',
      }}>AH</div>

      {/* thin vertical rule */}
      <div style={{ position:'absolute', top:0, left:'48px', width:'1px', height:'100%', background:'linear-gradient(to bottom, transparent, var(--grey1) 30%, var(--grey1) 70%, transparent)', opacity:.7 }}/>
      ...
      <div style={{ position:'absolute', top:'80px', right:'48px', textAlign:'right', fontFamily:'var(--mono)', fontSize:'10px', color:'var(--grey2)', lineHeight:2.2 }}>
        <div>40.7128° N</div>
        <div>74.0060° W</div>
        <div style={{ color:'var(--grey3)', marginTop:'4px' }}>SYS.ONLINE</div>
      </div>
      ...
      <div style={{ maxWidth:'960px', position:'relative', zIndex:1 }}>
        <div ref={r1} className="rv" style={{ transitionDelay:'.08s', marginBottom:'24px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', fontFamily:'var(--body)', fontSize:'11px', fontWeight:400, letterSpacing:'.3em', textTransform:'uppercase', color:'var(--grey3)' }}>
            <div style={{ width:'36px', height:'1px', background:'var(--grey2)' }}/>
            4-Angle AI Hair Consultation
          </div>
        </div>
        ...
        <div ref={r3} className="rv" style={{ transitionDelay:'.32s', marginTop:'44px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', flexWrap:'wrap', gap:'28px' }}>
          <p style={{ fontFamily:'var(--body)', fontSize:'clamp(15px,1.3vw,18px)', color:'var(--grey3)', maxWidth:'420px', lineHeight:1.8, fontWeight:300 }}>
            Upload four photos of your head — front, back, side, and top. Our AI reads all four angles and engineers a hairstyle that fits your exact geometry, with full hairline coverage.
          </p>
          <button onClick={onBegin}
            style={{
              fontFamily:'var(--display)', fontSize:'13px', fontWeight:600, letterSpacing:'.2em',
              textTransform:'uppercase', color:'var(--white)', background:'var(--black)',
              padding:'15px 48px', transition:'all .25s', flexShrink:0,
            }}
      ...
      {!image ? (
        <>
          <div
            ...
            style={{
              aspectRatio:'3/4',
              border:`1px dashed ${drag ? 'var(--grey4)' : 'var(--grey1)'}`,
              background: drag ? 'var(--mist)' : 'var(--off)',
          ...
          >
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--grey2)" strokeWidth="1.5">
              <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
              <polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/>
            </svg>
            <span style={{ fontFamily:'var(--body)', fontSize:'10px', fontWeight:400, letterSpacing:'.15em', textTransform:'uppercase', color:'var(--grey2)' }}>Drop or click</span>
          </div>
        ...
        <div style={{ position:'relative', aspectRatio:'3/4', overflow:'hidden', border:'1px solid var(--grey1)' }}>
          ...
          <div style={{ position:'absolute', inset:0, background:'linear-gradient(to top,rgba(0,0,0,.3) 0%,transparent 40%)', pointerEvents:'none' }}/>
          {/* scan line */}
          <div style={{ position:'absolute', left:0, right:0, height:'1px', background:'linear-gradient(90deg,transparent,rgba(17,17,16,.5),transparent)', animation:'scan 2.5s ease-in-out infinite' }}/>
          <div style={{ position:'absolute', bottom:'8px', left:'8px', right:'8px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <span style={{ fontFamily:'var(--mono)', fontSize:'9px', color:'rgba(255,255,255,.7)', letterSpacing:'.1em' }}>✓</span>
            <button
              onClick={() => onImage(id, null)}
              style={{ fontFamily:'var(--body)', fontSize:'9px', fontWeight:400, letterSpacing:'.15em', textTransform:'uppercase', color:'rgba(255,255,255,.6)', background:'rgba(0,0,0,.4)', padding:'3px 8px', transition:'color .15s' }}
    ...
    <section id="upload" ref={uploadRef} style={{
      ...
      <div ref={r} className="rv">
        <div style={{ display:'flex', alignItems:'center', gap:'16px', marginBottom:'10px' }}>
          ...
          <h2 style={{ fontFamily:'var(--display)', fontSize:'clamp(24px,3vw,40px)', fontWeight:700, textTransform:'uppercase', letterSpacing:'.04em', color:'var(--black)' }}>Upload 4 Angles</h2>
        </div>
        <p style={{ fontFamily:'var(--body)', fontSize:'15px', fontWeight:300, color:'var(--grey3)', lineHeight:1.75, marginBottom:'48px', maxWidth:'480px' }}>
          All four views are required — front, back, side, and top. Same person, same session.
        </p>
        ...
        {/* progress bar */}
        <div style={{ height:'2px', background:'var(--mist)', marginBottom:'28px', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'16px' }}>
          ...
          <button
            ...
            style={{
              fontFamily:'var(--display)', fontSize:'12px', fontWeight:600, letterSpacing:'.2em',
              textTransform:'uppercase', padding:'14px 52px', transition:'all .25s',
              color: allFilled && !running ? 'var(--white)' : 'var(--grey2)',
              background: allFilled && !running ? 'var(--black)' : 'var(--mist)',
              border:'none',
            }}
...
const Scanner = ({ images }) => {
  const LOG = [
    'LOADING FACIAL MESH ENGINE',
    'MAPPING FRONT GEOMETRY',
    'ANALYSING BACK CONTOUR',
    'MEASURING SIDE PROFILE',
    'READING CROWN TOPOLOGY',
    'COMPUTING HEAD SHAPE INDEX',
    'HAIRLINE COVERAGE ALGORITHM',
    'SELECTING OPTIMAL HAIRSTYLE',
    'HAIRLINE COVERAGE CONFIRMED',
  ...
  useEffect(() => {
    const t = setInterval(() => setIdx(i => (i+1)%LOG.length), 700);
    return () => clearInterval(t);
  }, []);

  return (
    <div style={{
      padding:'60px 48px 80px', maxWidth:'900px', margin:'0 auto',
      textAlign:'center', animation:'fadeIn .5s ease',
      background:'var(--white)',
    }}>
      <div style={{ fontFamily:'var(--body)', fontSize:'11px', fontWeight:400, letterSpacing:'.28em', textTransform:'uppercase', color:'var(--grey3)', marginBottom:'40px' }}>
        Neural Processing · 4-Angle Analysis
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'8px', marginBottom:'32px' }}>
        {['front','back','side','top'].map((k,i) => (
          <div key={k} style={{ position:'relative', aspectRatio:'3/4', overflow:'hidden', border:'1px solid var(--mist)', background:'var(--light)' }}>
            ...
            <div style={{ position:'absolute', left:0, right:0, height:'1px', background:'linear-gradient(90deg,transparent,rgba(17,17,16,.7),transparent)', animation:`scan 2s ease-in-out infinite`, animationDelay:`${i*.3}s` }}/>
      ...
      <div style={{
        background:'var(--off)', border:'1px solid var(--mist)',
        padding:'14px 20px', maxWidth:'420px', margin:'0 auto', textAlign:'left',
      }}>
        <div style={{ fontFamily:'var(--mono)', fontSize:'11px', color:'var(--grey4)', letterSpacing:'.04em', minHeight:'18px' }}>
          ...
          {LOG[idx]}
          <span style={{ animation:'blink .8s infinite', marginLeft:'2px' }}>_</span>
        </div>
      </div>
    </div>
  );
...
const ErrorBanner = ({ msg, onRetry, onDismiss }) => (
  <div style={{ maxWidth:'960px', margin:'0 auto', padding:'0 48px 32px', animation:'fadeUp .3s ease' }}>
    <div style={{ padding:'18px 22px', border:'1px solid #e5b4b4', background:'#fdf5f5', display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'16px' }}>
      ...
      <div style={{ display:'flex', gap:'8px', flexShrink:0 }}>
        ...
        <button onClick={onDismiss} style={{ fontFamily:'var(--body)', fontSize:'10px', fontWeight:400, letterSpacing:'.1em', color:'var(--grey3)', padding:'8px', transition:'color .15s' }}
  ...
  return (
    <section id="results" ref={resultsRef} style={{ padding:'80px 48px 100px', maxWidth:'1200px', margin:'0 auto', background:'var(--white)' }}>
      <div ref={r} className="rv">
        ...
        {/* ── Before → After grid ──────────────────────────────────────── */}
        <div style={{ fontFamily:'var(--body)', fontSize:'10px', fontWeight:400, letterSpacing:'.22em', textTransform:'uppercase', color:'var(--grey2)', marginBottom:'16px' }}>
          Before → After · 4 Angles
        </div>
              ...
              <div key={key} style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
                ...
                <div
                  ...
                  style={{
                    ...
                    border:`1px solid ${gen?.url ? 'var(--grey2)' : 'var(--mist)'}`,
                    background:'var(--off)', cursor: gen?.url ? 'none' : 'default',
                ...
                >
                  {gen?.url ? (
                    <>
                      ...
                      <div style={{ position:'absolute', bottom:'5px', right:'5px', fontFamily:'var(--mono)', fontSize:'8px', color:'rgba(0,0,0,.35)', background:'rgba(255,255,255,.7)', padding:'2px 5px', letterSpacing:'.08em' }}>↗</div>
                    </>
                  ) : gen?.error ? (
                ...
                <div style={{ fontFamily:'var(--body)', fontSize:'9px', fontWeight:500, letterSpacing:'.18em', textTransform:'uppercase', color:'var(--grey3)', textAlign:'center' }}>
                  {LABEL[key]}
                </div>
              </div>
            );
        ...
        {analysis && (
          <div style={{
            background:'linear-gradient(135deg,var(--off) 0%,var(--light) 100%)',
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:'16px', marginBottom:'20px' }}>
              <div>
                <div style={{ fontFamily:'var(--body)', fontSize:'10px', fontWeight:500, letterSpacing:'.3em', textTransform:'uppercase', color:'var(--grey3)', marginBottom:'8px' }}>
                  Recommended Hairstyle
                </div>
                <h3 style={{ fontFamily:'var(--display)', fontSize:'clamp(22px,2.6vw,34px)', fontWeight:700, textTransform:'uppercase', letterSpacing:'.04em', color:'var(--black)' }}>
                  {analysis.hairstyle_name}
                </h3>
              </div>
              <div style={{ fontFamily:'var(--mono)', fontSize:'10px', color:'var(--grey4)', background:'var(--white)', padding:'7px 14px', border:'1px solid var(--grey1)', letterSpacing:'.08em', alignSelf:'flex-start' }}>
                {analysis.length}
              </div>
            </div>

            <p style={{ fontFamily:'var(--body)', fontSize:'15px', fontWeight:300, color:'var(--grey4)', lineHeight:1.85, marginBottom:'24px', maxWidth:'680px' }}>
              {analysis.description}
            </p>

            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))', gap:'10px', marginBottom:'22px' }}>
              {[
                { label:'Front', text: analysis.front_notes },
                { label:'Back',  text: analysis.back_notes },
                { label:'Side',  text: analysis.side_notes },
                { label:'Top',   text: analysis.top_notes },
              ].map(({ label, text }) => text && (
...
export default function App() {
  ...
  const handleImage = useCallback((id, val) => {
  ...
  const run = async () => {
    ...
    setPhase('running');
    setErr(null); setAnalysis(null); setResults([]);
    ...
    try {
      const r = await post('/api/generate', { front:images.front, back:images.back, side:images.side, top:images.top, analysis:analysisData }, 220_000);
      setResults(r.results || []);
    } catch(e) {
      console.warn('[App] generation error:', e.message);
      setResults([]);
    }
  ...
  return (
    <>
      ...
      {phase !== 'done' && (
        <Upload images={images} onImage={handleImage} uploadRef={uploadRef} onRun={run} running={phase==='running'}/>
      )}
      ...
      {phase === 'done' && (
        <Results analysis={analysis} results={results} inputImages={savedImgs.current} onReset={reset} resultsRef={resultsRef}/>
      )}

      <footer style={{ borderTop:'1px solid var(--mist)', padding:'36px 48px', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'16px', background:'var(--off)' }}>
        <div style={{ fontFamily:'var(--display)', fontSize:'13px', fontWeight:700, letterSpacing:'.2em', textTransform:'uppercase', color:'var(--black)' }}>
          American Hairline
        </div>
